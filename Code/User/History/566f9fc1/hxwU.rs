	...
	pub fn process_input_event<I: InputBackend>(&mut self, event: InputEvent<I>) {
		match event {
			...
			InputEvent::PointerAxis { event, .. } => {
				...
				let horizontal_amount_v120 = event.amount_v120(input::Axis::Horizontal);
				let vertical_amount_v120 = event.amount_v120(input::Axis::Vertical);

				{
					...
					if horizontal_amount != 0.0 {
						...
						if let Some(discrete) = horizontal_amount_v120 {
							frame = frame.amount_v120(Axis::Horizontal, discrete as i32);
						}
