...
export default makeScene2D(function* (view) {
    ...
    yield* chain(
        ...
        tween(0.75, v => {
            subscribe().scale(map(0, 1, easeInQuad(v)))
        }),
        tween(0.75, v => {
            video().scale(map(0, 1, easeInQuad(v)))
        }),
        tween(0.75, v => {
            playlist().scale(map(0, 1, easeInQuad(v)))
        }),
        waitFor(1.2),
        tween(0.75, v => {
            sub_curve().opacity(map(0, 1, easeInQuad(v)))
            sub_curve_text().opacity(map(0, 1, easeInQuad(v)))
        }),
    );
