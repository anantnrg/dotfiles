...
export default makeScene2D(function* (view) {
    ...
    yield* chain(
        ...
        all(
            tween(0.55, value => {
                dir_highlight_ref().x(map(25, 5, easeOutSine(value)))
                dir_highlight_ref().y(map(160, 3, easeOutSine(value)))
                dir_highlight_ref().width(map(430, 350, easeOutSine(value)))
                dir_highlight_ref().x(map(200, 100, easeOutSine(value)))
            }),
        )
    );

    yield dir_highlight_ref().x(55);
    yield dir_highlight_ref().y(3);
    yield dir_highlight_ref().opacity(1);
    yield dir_highlight_ref().scale(0);
    yield dir_highlight_ref().width(350);
    yield dir_highlight_ref().height(100);

    yield* waitUntil("start-entry-point");

    yield* chain(
        tween(0.55, value => {
            dir_highlight_ref().scale(map(0, 1, easeOutSine(value)))
        }),
        all(
            tween(0.55, value => {
