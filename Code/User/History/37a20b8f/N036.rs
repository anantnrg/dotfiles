...
use freya::{
    hooks::Line,
    torin::{
        custom_measurer::LayoutMeasurer, dom_adapter::DOMAdapter, geometry::Size2D, node::Node, size::Size,
    },
};
use ropey::RopeSlice;
                ...
                NodeType::Line { chars } => {
                    ...
                    for char in chars {
                        let text_bounds = self
                            .font
                            .measure_text(char.to_string().as_str(), Some(&self.paint))
                            .1;
                    }
                }
                NodeType::Char(char) => {}
...
pub fn measure_char(char: RopeSlice<'_>, font: &Font, paint: &Paint) -> Size<f32> {
